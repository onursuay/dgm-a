  ...
  return (
    <>
      ...
      >
        <div className="max-w-[1300px] mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
              ...
              {languages.map((l) => (
                <button key={l} onClick={() => setLang(l)}
                  className={`px-3 py-1 text-xs font-semibold rounded-full transition-all ${
                    lang === l ? "bg-gradient-to-r from-blue-600 to-blue-800 text-white" : "text-[#3a5a7a] hover:text-[#6688aa]"
                  }`}>
                  {l}
                </button>
              ))}
          ...
          <button
            ...
            <span className="block w-6 h-0.5 bg-[#38b6ff]" />
            <span className="block w-4 h-0.5 bg-[#38b6ff]" />
            <span className="block w-6 h-0.5 bg-[#38b6ff]" />
          </button>
        </div>
      </header>
      ...
      <div
        ...
        style={{ background: "rgba(3,8,20,0.98)", backdropFilter: "blur(24px)", borderRight: "1px solid rgba(26,111,255,0.12)" }}
      >
        {/* Drawer header */}
        <div className="flex items-center justify-between px-6 h-16 shrink-0" style={{ borderBottom: "1px solid rgba(26,111,255,0.12)" }}>
          <a href="#home" className="flex items-center gap-3" onClick={() => setMenuOpen(false)}>
            ...
            <span className="text-lg font-black tracking-widest text-[#e8f0ff]">
              D<span className="text-ice-gradient">N</span>F
            </span>
          </a>
          <button
            onClick={() => setMenuOpen(false)}
            className="w-8 h-8 flex items-center justify-center rounded-full transition-colors hover:bg-blue-900/20"
            aria-label="Close menu">
            <svg className="w-5 h-5 text-[#3a5a7a]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-4">
          {navLinks.map((link) => (
            <a
              ...
              href={link.href}
              onClick={() => setMenuOpen(false)}
              className="flex items-center gap-3 px-3 py-3.5 rounded-xl text-sm font-medium text-[#5e7a9a] hover:text-[#38b6ff] hover:bg-blue-900/10 transition-all duration-200"
              style={{ borderBottom: "1px solid rgba(26,111,255,0.06)" }}
            >
              {link.label}
            </a>
          ))}
        ...
        {/* Drawer footer */}
        <div className="shrink-0 px-4 py-5 space-y-3" style={{ borderTop: "1px solid rgba(26,111,255,0.12)" }}>
          {/* Language */}
          <div className="flex items-center gap-1 p-1 rounded-full" style={{ background: "rgba(26,111,255,0.06)", border: "1px solid rgba(26,111,255,0.12)" }}>
            {languages.map((l) => (
              <button key={l} onClick={() => setLang(l)}
