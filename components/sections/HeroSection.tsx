  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        ctx.save(); ctx.globalAlpha = p.alpha; ctx.fillStyle = "#1a6fff";
        ctx.shadowBlur = 6; ctx.shadowColor = "#1a6fff";
  ...
  return (
    <section id="home" className="relative min-h-screen flex flex-col justify-center items-center overflow-hidden pt-20"
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        backgroundImage: "linear-gradient(rgba(26,111,255,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(26,111,255,0.05) 1px, transparent 1px)",
        backgroundSize: "64px 64px"
      }} />

      {/* Center glow */}
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "radial-gradient(ellipse 70% 60% at 50% 50%, rgba(26,111,255,0.1) 0%, transparent 70%)"
      }} />
      ...
      <div className="relative z-10 max-w-[1100px] mx-auto px-6 text-center w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-blue-900/20 border border-blue-900/30 rounded-2xl overflow-hidden">
          {[
            ...
            <div key={s.label} className="py-6 px-4 text-center" style={{ background: "rgba(4,14,32,0.8)" }}>
              <div className="text-3xl font-black mb-1" style={{
                background: "linear-gradient(135deg, #38b6ff 0%, #1a6fff 100%)",
                WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text"
              }}>{s.value}</div>
              <div className="text-xs font-bold text-[#c8ddf5] mb-0.5">{s.label}</div>
              <div className="text-[10px] text-[#3a5a7a]">{s.sub}</div>
            </div>
          ))}
        </div>
      </div>
