        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <div className="grid grid-cols-4 gap-3 mb-8">
              {targets.map((t) => (
                <div key={t.label} className="text-center border border-orange-900/15 rounded-xl p-3 bg-orange-950/5 hover:border-orange-700/30 transition-colors">
                  <div className="w-8 h-8 mx-auto mb-2 text-[#ff6b00]">
                    <t.Icon className="w-8 h-8" />
                  </div>
                  <div className="text-xs text-[#6a6060]">{t.label}</div>
                </div>
              ))}
            </div>
          ...
          <div className="relative">
            <div className="relative rounded-3xl overflow-hidden border border-orange-900/20"
              style={{ background: "linear-gradient(135deg, #0f0800 0%, #1a0e00 50%, #0f0800 100%)", minHeight: "480px" }}>
              <div className="absolute bottom-0 left-0 right-0 h-48 pointer-events-none"
                style={{ background: "linear-gradient(to top, rgba(180,40,0,0.2) 0%, transparent 100%)" }} />
              <div className="absolute bottom-0 left-0 right-0 overflow-hidden">
                <svg viewBox="0 0 800 300" className="w-full" preserveAspectRatio="none">
                  <rect x="20" y="180" width="60" height="120" fill="#1a1a1a" stroke="rgba(255,107,0,0.1)" strokeWidth="1"/>
                  <rect x="35" y="160" width="30" height="20" fill="#1a1a1a"/>
                  <rect x="100" y="140" width="80" height="160" fill="#222" stroke="rgba(255,107,0,0.1)" strokeWidth="1"/>
                  <rect x="120" y="120" width="40" height="20" fill="#222"/>
                  <rect x="200" y="160" width="50" height="140" fill="#1e1e1e"/>
                  <rect x="270" y="100" width="90" height="200" fill="#252525" stroke="rgba(255,107,0,0.08)" strokeWidth="1"/>
                  ...
                  {[[110,150],[130,150],[150,150],[110,170],[130,170],[150,170],[280,110],[300,110],[320,110],[280,130],[300,130],[480,130],[500,130],[520,130],[480,150],[500,150]].map(([x,y], i) => (
                    <rect key={i} x={x} y={y} width="8" height="6" fill={i % 3 === 0 ? "rgba(255,150,0,0.5)" : "rgba(255,200,100,0.3)"} rx="1"/>
                  ))}
                  <path d="M380 0 L450 30 L450 100 Q450 160 390 190 Q330 160 330 100 L330 30 Z" fill="none" stroke="rgba(255,107,0,0.15)" strokeWidth="1.5"/>
                  <rect x="0" y="295" width="800" height="5" fill="rgba(255,107,0,0.15)"/>
                </svg>
              </div>
              <div className="relative z-10 p-10 text-center">
                ...
                <div className="text-3xl font-black text-[#f5f0eb] mb-2">Urban Shield</div>
                <div className="text-[#ff9500] text-sm font-medium">DNF Municipal Protection Program</div>
                <div className="mt-8 grid grid-cols-3 gap-4">
                  {[{ v: "2,400+", l: "Buildings Protected" }, { v: "12 Cities", l: "Active Deployments" }, { v: "100%", l: "Eco-Compliant" }].map((s) => (
                    <div key={s.l} className="text-center">
                      ...
                      <div className="text-xs text-[#6a6060]">{s.l}</div>
                    </div>
                  ))}
                </div>
              </div>
